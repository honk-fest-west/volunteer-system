<script lang="ts">
  import { User } from "$types";

  export let volunteers: User[];

  // Create a row of volunteer data for the CSV content
  function createVolunteerRow(volunteer: User) {
    return [
      volunteer.displayName.split(" ")[0],
      volunteer.displayName.split(" ")[1],
      volunteer.email,
      volunteer.role,
    ];
  }

  // Create all the rows related to a specific job for the CSV content

  // Create the CSV content from the event and sign-ups data
  function createCsvContent() {
    const rows = [
      volunteers.reduce(
        (acc, volunteer) => {
          return [...acc, ...createVolunteerRow(volunteer)];
        },
        [] as Array<string[]>
      ),
    ];

    return rows.map((e) => e.join(",")).join("\n");
  }

  // Create a Blob containing the CSV content from the event and sign-ups data
  function createCsvBlob() {
    const csvContent = createCsvContent();
    return new Blob([csvContent], { type: "text/csv;charset=utf-8" });
  }

  // Download the CSV file by creating a temporary link and simulating a click
  function downloadCsv(blob: Blob) {
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = "volunteer-data.csv";
    link.click();
    URL.revokeObjectURL(url);
  }

  // Generate and download the CSV file containing event and sign-ups data
  function generate() {
    const csvBlob = createCsvBlob();
    downloadCsv(csvBlob);
  }
</script>

<button
  on:click={generate}
  class="text-indigo-700
         font-semibold
         opacity-70
         hover:opacity-100
         flex
         items-center"
>
  <span>
    <svg
      xmlns="http://www.w3.org/2000/svg"
      class="h-6 w-6"
      fill="none"
      viewBox="0 0 24 24"
      stroke="currentColor"
      stroke-width="2"
    >
      <path
        stroke-linecap="round"
        stroke-linejoin="round"
        d="M3 10h18M3 14h18m-9-4v8m-7 0h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"
      />
    </svg>
    &nbsp;spreadsheet
  </span>
</button>
